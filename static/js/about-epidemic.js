window.onload = function () {
	...
	oUpdate.onclick = function () {
		if (dicContentInfo["newstype"] === "news") {
			...
			fnTransferData("POST", "/", dicNewsInfo, fnAfterGetNews);
		}

		if (dicContentInfo["newstype"] === "rumors") {
			dicRumorsInfo["page"] = Number(dicRumorsInfo["page"]) + 1;
			fnTransferData("POST", "/", dicRumorsInfo, fnAfterGetRumors);
		}
		...
		for (let i = 0; i < arrNewsChoice.length; i++) {
			arrNewsChoice[i].onclick = function () {
				...
				if (this.innerText === "辟谣") {
					...
					oUpdate.style.color = "#fff";
					if (oDataFlag === 0) {
						fnTransferData("POST", "/", dicContentInfo, fnAfterGetRumors);
					}
					flag += 1;
				}
				if (this.innerText === "地图") {
					...
					fnTransferData("POST", "/", dicContentInfo, fnAfterGetEpidemicData);
				}
	...
	function fnAfterGetNews(oGetData) {
		...
		fnTransferData("POST", "/", dicOverall, fnAfterGetEpidemicOverall);

	}

	fnTransferData("POST", "/", dicContentInfo, fnAfterGetNews);
